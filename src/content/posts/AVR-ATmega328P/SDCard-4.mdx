---
title: ATmega328P SDCard 구현 (4)
author: KyuHyuk Lee
pubDatetime: 2022-09-03T04:32:01.000Z
slug: sdcard-4
featured: false
draft: false
tags:
  ...
  - ATmega328P
  - SDCard
description: ATmega328P의 SDCard SPI 초기화 4편 입니다. CMD16(SET_BLOCKLEN), CMD17(READ_SINGLE_BLOCK), CMD24(WRITE_BLOCK)를 사용한 SDCard 데이터 읽기/쓰기 구현, 시작 토큰(0xFE), 데이터 응답 토큰, 에러 토큰과 Timeout 처리에 대해 설명합니다.
---

import sdCardSpecification from "@/assets/files/AVR-ATmega328P-SDCard/Part_1_Physical_Layer_Simplified_Specification_Ver2.00_060925.pdf";

[이전 글](/posts/avr-atmega328p/sdcard-3)에서 SDCard 초기화 과정을 구현했습니다. 이 글에서는 SDCard에 데이터를 읽고 쓰는 방법을 구현합니다.
...
![CMD17 Format](@/assets/images/AVR-ATmega328P/SDCard-4/AVR-ATmega328P-SDCard-4_1.png)
...
![Single Block Read Operation](@/assets/images/AVR-ATmega328P/SDCard-4/AVR-ATmega328P-SDCard-4_2.png)
...
![Single Block - Start Token](@/assets/images/AVR-ATmega328P/SDCard-4/AVR-ATmega328P-SDCard-4_3.png)
...
<a href={sdCardSpecification}>SDCard Physical Specification</a>의 4.6.2.1 섹션을 보면 아래와 같은 내용이 있습니다:
...
[이전 글](/posts/avr-atmega328p/sdcard-3)의 초기화 코드를 사용하여 SDCard에서 맨 처음 블록을 읽어 보겠습니다.
...
UART에 아래와 같이 출력됩니다:  
![UART - Read Single Block](@/assets/images/AVR-ATmega328P/SDCard-4/AVR-ATmega328P-SDCard-4_4.png)
...
![Logic Analyzer - Read Single Block](@/assets/images/AVR-ATmega328P/SDCard-4/AVR-ATmega328P-SDCard-4_5.png)
...
![Read Operation - Data Error](@/assets/images/AVR-ATmega328P/SDCard-4/AVR-ATmega328P-SDCard-4_6.png)

데이터 오류 토큰의 형식은 아래와 같습니다.

![Data Error Token](@/assets/images/AVR-ATmega328P/SDCard-4/AVR-ATmega328P-SDCard-4_7.png)
...
![SanDisk SDCard](@/assets/images/AVR-ATmega328P/SDCard-4/AVR-ATmega328P-SDCard-4_8.png)
...
![Samsung SDCard](@/assets/images/AVR-ATmega328P/SDCard-4/AVR-ATmega328P-SDCard-4_9.png)

혹시 코드가 잘못되었나 확인하기 위해 Logic Analyzer로 확인해 봤지만, 제가 가진 SDCard에서는 오류 토큰을 받은 적이 없으며 ~100ms 후에 Timeout이 되었습니다.  
![Logic Analyzer - Read Single Block](@/assets/images/AVR-ATmega328P/SDCard-4/AVR-ATmega328P-SDCard-4_10.png)

# `CMD24` (WRITE_BLOCK)

읽기와 마찬가지로 쓰기도 512바이트 블록에서 수행됩니다. `CMD24`를 사용하여 쓰기를 수행합니다. 이 명령의 형식은 아래와 같습니다.

![CMD24](@/assets/images/AVR-ATmega328P/SDCard-4/AVR-ATmega328P-SDCard-4_11.png)

단일 블록에 쓰는 방법은 아래와 같습니다.

![Single Block Write Operation](@/assets/images/AVR-ATmega328P/SDCard-4/AVR-ATmega328P-SDCard-4_12.png)
...
![Data Response Token](@/assets/images/AVR-ATmega328P/SDCard-4/AVR-ATmega328P-SDCard-4_13.png)
...
읽기 작업과 마찬가지로 타임아웃 값을 설정합니다. <a href={sdCardSpecification}>SDCard Physical Specification</a>의 4.6.2.2 섹션을 보면 모든 쓰기 작업에 대해 타임아웃은 250ms로 정의해야 한다고 언급하고 있습니다. `(0.25s * 16000000 Hz) / (128 * 8 Bytes) = 3906.25`이지만, `3907`로 250ms보다 조금 크게 설정했습니다.
...
![sdWriteSingleBlock UART](@/assets/images/AVR-ATmega328P/SDCard-4/AVR-ATmega328P-SDCard-4_14.png)

Logic Analyzer로 확인해 보면 `R1`으로 응답한 뒤 바로 시작 토큰을 보내고, 기록할 데이터 전송을 시작합니다.  
![Logic Analyzer](@/assets/images/AVR-ATmega328P/SDCard-4/AVR-ATmega328P-SDCard-4_15.png)
...
![Logic Analyzer](@/assets/images/AVR-ATmega328P/SDCard-4/AVR-ATmega328P-SDCard-4_16.png)
