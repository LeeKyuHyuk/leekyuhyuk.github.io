---
title: ATmega328P SDCard 구현 (2)
author: KyuHyuk Lee
pubDatetime: 2022-09-01T22:50:19.000Z
slug: sdcard-2
featured: false
draft: false
tags:
  ...
  - ATmega328P
  - SDCard
description: ATmega328P의 SDCard SPI 초기화 2편 입니다. CMD8(Version 확인), CMD58(OCR 읽기), R7/R3 응답 처리, UART 디버깅 출력, CLI 구현까지 초기화 흐름의 후반부를 C 코드와 함께 설명합니다.
---
...
[이전 글](/posts/avr-atmega328p/sdcard-1)에서 `CMD0`를 사용해서 SDCard를 IDLE 상태로 만들었습니다. 이 글에서는 그 이후의 초기화 과정을 소개하고 구현해 볼 것입니다.

![SDCard SPI Mode Initalization Flow](@/assets/images/AVR-ATmega328P/SDCard-1/AVR-ATmega328P-SDCard-1_5.png)
...
![SDCard SPI Mode Initalization Flow](@/assets/images/AVR-ATmega328P/SDCard-2/AVR-ATmega328P-SDCard-2_1.png)

`CMD8`를 전송하기 위해 Command Index를 `8 (001000b)`로 설정합니다. 3.3V에서 작동하므로 `VHS`는 `0001b`로 설정합니다. Check Pattern은 아무 값이나 설정이 가능하며, SDCard는 명령이 올바르게 처리되었는지 확인하기 위해 응답으로 반환합니다. 참고로 <a href={sdCardSpecification}>SDCard Physical Specification</a> 51페이지를 보면 `10101010b`를 권장하고 있습니다. 그리고 마지막으로 CRC를 설정해야 하는데 `CMD8`은 올바른 CRC가 필요한 `CMD0` 이외의 유일한 명령입니다. 3.3V `VHS`를 사용하고 권장되는 Check Pattern를 Command에 지정된 대로 모든 비트를 설정하면 올바른 CRC는 `01000011b` 입니다. 아래와 같이 `CMD8` 명령을 전송할 수 있습니다.
...
![CRC7-MMC Calculator](@/assets/images/AVR-ATmega328P/SDCard-2/AVR-ATmega328P-SDCard-2_2.png)
...
![R7 Response Format](@/assets/images/AVR-ATmega328P/SDCard-2/AVR-ATmega328P-SDCard-2_3.png)
...
![UART Output](@/assets/images/AVR-ATmega328P/SDCard-2/AVR-ATmega328P-SDCard-2_4.png)

정상적으로 동작하는 것을 확실하게 보고 싶다면 Logic Analyzer로 `CMD8`에 대한 Output을 확인해 보는 것도 좋습니다.  
![Logic Analyzer](@/assets/images/AVR-ATmega328P/SDCard-2/AVR-ATmega328P-SDCard-2_5.png)
...
![CMD58 Format](@/assets/images/AVR-ATmega328P/SDCard-2/AVR-ATmega328P-SDCard-2_6.png)

Argument는 Stuff Bit이고 CRC는 무시해도 됩니다. Response는 `R3` 형식으로 받으며 아래와 같습니다.  
![R3 Format](@/assets/images/AVR-ATmega328P/SDCard-2/AVR-ATmega328P-SDCard-2_7.png)

`R7`과 마찬가지로 첫 번째 바이트는 `R1`과 동일하고, 다음 4바이트는 OCR이 담겨있습니다. OCR은 아래와 같이 정의되어 있습니다.  
![OCR Register Definition](@/assets/images/AVR-ATmega328P/SDCard-2/AVR-ATmega328P-SDCard-2_8.png)
...
![SDCard CLI](@/assets/images/AVR-ATmega328P/SDCard-2/AVR-ATmega328P-SDCard-2_9.png)
