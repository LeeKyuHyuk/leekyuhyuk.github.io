---
title: ATmega328P SDCard 구현 (1)
author: KyuHyuk Lee
pubDatetime: 2022-08-28T16:49:27.000Z
slug: sdcard-1
featured: false
draft: false
tags:
  ...
  - ATmega328P
  - SDCard
description: ATmega328P의 SDCard SPI 초기화, SPI 설정, 핀 맵핑, Power Up 시퀀스(80 Clocks), CMD0 전송과 R1 응답 처리 등 SDCard IDLE 상태 진입 전 과정을 C 코드와 함께 설명합니다.
---
...
import secureDigitalCard from "@/assets/files/AVR-ATmega328P-SDCard/SecureDigitalCard_1.9.pdf";

SPI(Serial Peripheral Iterface) 모드에서 ATmega328P를 사용하여 SDCard를 초기화하는 방법을 소개합니다. SPI에 익숙하지 않은 경우에는 [이 글](/posts/avr-atmega328p/spi)을 읽어보시기 바랍니다.

이 글에서 제공하는 정보의 대부분은 <a href={sdCardSpecification}>SDCard Physical Specification</a>를 참고하였습니다.

# 회로 연결

아래와 같이 회로를 구성합니다. Arduino를 연결한 이유는 `avrdude`를 사용하여 ATmega328P의 Flash에 프로그래밍(Flash Write)하기 위해 연결했습니다.

![Connections](@/assets/images/AVR-ATmega328P/SDCard-1/AVR-ATmega328P-SDCard-1_1.png)

# SPI 초기화

SPI를 설정하려면 클럭의 극성, 위상 및 속도와 같은 몇 가지 설정이 필요합니다. 아쉽게도 버스 타이밍 다이어그램은 SDCard Physical Specification에는 없습니다. 그러나 다행이게도 <a href={secureDigitalCard}>SanDisk SDCard 제품 설명서</a>에는 아래 타이밍 다이어그램이 제공됩니다.

![Timing Diagram Data Input/Output Referenced to Clock](@/assets/images/AVR-ATmega328P/SDCard-1/AVR-ATmega328P-SDCard-1_2.png)
...
![Power-up Diagram](@/assets/images/AVR-ATmega328P/SDCard-1/AVR-ATmega328P-SDCard-1_3.png)
...
![Command Format](@/assets/images/AVR-ATmega328P/SDCard-1/AVR-ATmega328P-SDCard-1_4.png)
...
![CMD0 Description](@/assets/images/AVR-ATmega328P/SDCard-1/AVR-ATmega328P-SDCard-1_6.png)
...
![R1 Response Format](@/assets/images/AVR-ATmega328P/SDCard-1/AVR-ATmega328P-SDCard-1_7.png)
...
![Logic Analyzer Setting](@/assets/images/AVR-ATmega328P/SDCard-1/AVR-ATmega328P-SDCard-1_8.png)

Logic Analyzer를 사용하면 아래와 같은 화면이 출력됩니다.

![Logic Analyzer](@/assets/images/AVR-ATmega328P/SDCard-1/AVR-ATmega328P-SDCard-1_9.png)
